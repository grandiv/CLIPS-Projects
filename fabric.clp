...
;;************************************
;;  COMBINE CERTAINTIES (Durkin, 1994)
;;************************************
...
(defrule rule8
  (and 
    (a-fact (name q3) (cf ?cf1))
    (a-fact (name q7) (cf ?cf2)))
  =>
  (if (and (>= ?cf1 0.4) (>= ?cf2 0.4)) 
       then 
      (assert (a-fact (name prefers-polyester) (cf (* 0.9 (min ?cf1 ?cf2)))))
      (assert (a-fact (name prefers-cotton) (cf (* 0.8 (min ?cf1 ?cf2)))))
       else (if (and (<= ?cf1 -0.4) (<= ?cf2 -0.4))
                then (assert (a-fact (name prefers-silk) (cf (* 0.6 (min ?cf1 ?cf2)))))
                     (assert (a-fact (name prefers-brocade) (cf (* 0.7 (min ?cf1 ?cf2))))))))

(defrule rule9
  (and 
    (a-fact (name q4) (cf ?cf1))
    (a-fact (name q5) (cf ?cf2)))
  =>
  (if (and (>= ?cf1 0.4) (>= ?cf2 0.4)) 
       then 
      (assert (a-fact (name prefers-brocade) (cf (* 0.9 (min ?cf1 ?cf2)))))
      (assert (a-fact (name prefers-leather) (cf (* 0.8 (min ?cf1 ?cf2)))))
      (assert (a-fact (name prefers-denim) (cf (* 0.7 (min ?cf1 ?cf2)))))
       else (if (and (<= ?cf1 -0.4) (<= ?cf2 -0.4))
                then (assert (a-fact (name prefers-linen) (cf (* 0.6 (min ?cf1 ?cf2)))))
                     (assert (a-fact (name prefers-cotton) (cf (* 0.6 (min ?cf1 ?cf2))))))))
...
(defrule rule10
  ...
  (if (and (>= ?cf1 0.4) (>= ?cf2 0.4)) 
       then 
      (assert (a-fact (name prefers-wool) (cf (* 0.9 (min ?cf1 ?cf2)))))
      (assert (a-fact (name prefers-fleece) (cf (* 0.7 (min ?cf1 ?cf2)))))
       else (if (and (<= ?cf1 -0.4) (<= ?cf2 -0.4))
                then (assert (a-fact (name prefers-silk) (cf (* 0.6 (min ?cf1 ?cf2)))))
                     (assert (a-fact (name prefers-jersey) (cf (* 0.6 (min ?cf1 ?cf2))))))))              
...
(defrule fabric-selection-rule-9
  (finished)
    (a-fact (name prefers-wool) (cf ?cf1))
    (a-fact (name prefers-fleece) (cf ?cf2))
  =>
  (if (>= (* 0.7 (min ?cf1 ?cf2)) 0.4)
      then 
      (printout  t "------------------------------------------------------" crlf)
      (printout  t "Wool Fleece Blend with cf " (* 0.7 (min ?cf1 ?cf2)) crlf)
      (printout  t "------------------------------------------------------" crlf)
      (printout  t crlf)))

(defrule fabric-selection-rule-10
  (finished)
    (a-fact (name prefers-silk) (cf ?cf1))
    (a-fact (name prefers-brocade) (cf ?cf2))
  =>
  (if (>= (* 0.6 (min ?cf1 ?cf2)) 0.4)
      then 
      (printout  t "------------------------------------------------------" crlf)
      (printout  t "Silk Brocade Blend with cf " (* 0.6 (min ?cf1 ?cf2)) crlf)
      (printout  t "------------------------------------------------------" crlf)
      (printout  t crlf)))